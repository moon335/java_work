...
public class OrderFrame extends JFrame{
	...
	public OrderFrame(LoginFrame mContext) {
	...
	private void addEventListener() {
		...
		addButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				UserDto uDto = userController.requestUserInfo(mContext.getLogindto().getUserId(), mContext.getLogindto().getPassword());
				ProdDto pDto = prodController.requestOneProduct(field1.getText());
		...
		orderButton.addActionListener(new ActionListener() {
			@Override
			public void actionPerformed(ActionEvent e) {
				ArrayList<OrderDto> list = orderController.requestCheckOrder(mContext.getLogindto().getId());
				if(list.size() == 0) {
					JOptionPane.showMessageDialog(rootPane, "주문 목록이 비어 있습니다.");
				} else {
					new CheckOrderFrame(mContext);
					dispose();
				}
			}
		});
